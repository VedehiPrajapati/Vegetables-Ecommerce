...
//           images: p.images?.map(img => img.secure_url) || [], 
//           unit: p.unit,
...
//                         currentMainImage === imgUrl ? 'opacity-100' : 'opacity-60'
//                       }`}
//                     />
//                   </motion.div>
//                 ))}
...
//               {/* Main Image */}
//               <div className="relative flex-1 aspect-square md:aspect-auto md:h-[500px] bg-white rounded-3xl overflow-hidden border border-border flex items-center justify-center order-1 md:order-2">
//                 {product.discount > 0 && (
...
//                   <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2 hover:bg-background rounded-lg transition-colors">
//                     <Minus className="w-5 h-5" />
//                   </button>
//                   <span className="text-xl font-bold min-w-[30px] text-center">{quantity}</span>
//                   <button onClick={() => setQuantity(quantity + 1)} className="p-2 hover:bg-background rounded-lg transition-colors">
...
import { useAuthStore } from "@/store/authStore";
import { motion, AnimatePresence } from "framer-motion";
import { Plus, Minus, Star, ShoppingCart, ChevronRight, Loader2, Leaf } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import api from "@/services/api";
import SemicircleFooter from "@/components/SemicircleFooter";
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="hidden lg:block w-72 border-r border-border bg-background sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
        ...
        <div className="flex-1 bg-background min-h-screen">
          ...
          {/* Breadcrumb */}
          <div className="border-b border-border">
            <div className="container mx-auto px-4 py-3 flex items-center gap-2 text-xs sm:text-sm opacity-70 overflow-x-auto whitespace-nowrap">
          ...
          <div className="container mx-auto px-4 py-6 md:py-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 flex flex-col md:flex-row gap-4">
                ...
                {/* ✅ MAIN IMAGE (SMALL + CENTERED) */}
                <div className="relative flex-1 flex justify-center">
                  <div className="relative w-full max-w-[580px] aspect-square bg-white rounded-3xl border flex items-center justify-center overflow-hidden">

                    {product.discount > 0 && (
                      <div className="absolute top-0 left-0 bg-primary text-white px-3 py-2 text-xs rounded-br-xl font-bold z-10">
                        {product.discount}% Off
                      </div>
                    )}
              ...
              <div className="lg:col-span-5 flex flex-col justify-center space-y-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm opacity-60">(4.8 / 5)</span>
                </div>

                <div>
                  {product.originalPrice > product.price && (
                    <span className="line-through opacity-40 text-lg">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-sm opacity-60">/ {product.unit}</span>
                  </div>
                </div>
                ...
                {/* ✅ MOBILE PRODUCT LIST BELOW DESCRIPTION */}
                <div className="lg:hidden mt-6">
                  <h2 className="font-semibold mb-3">More Products</h2>
                  <div className="flex gap-3 overflow-x-auto">
                    {visibleProducts.map((p: any) => (
                      <div
                        key={p._id}
                        onClick={() => navigate(`/product/${p.slug}`)}
                        className="min-w-[140px] border rounded-xl p-2 cursor-pointer"
                      >
                        <img
                          src={p.mainImage?.secure_url}
