...
const Testimonials = () => {
  ...
  useEffect(() => {
    // Only set up the interval if the screen is mobile (less than 768px)
    const handleResize = () => {
      if (window.innerWidth >= 768) {
        setActiveIndex(0); // Reset to first position on desktop
      }
    };

    const interval = setInterval(() => {
      // Only advance the slide if we are on a small screen
      if (window.innerWidth < 768) {
    ...
    <section className="py-14 bg-gray-50 overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="relative">
          {/* On Mobile: flex + translateX 
              On Desktop (md:): grid + no transform 
          */}
          <div
          ...
          >
            {testimonials.map((item, index) => (
              <div
                key={index}
                className="w-full flex-shrink-0 px-2 md:px-0 md:w-auto"
              >
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  activeIndex === i ? "w-6 bg-green-500" : "w-2 bg-gray-300"
                }`}
              />
            ))}
          </div>
